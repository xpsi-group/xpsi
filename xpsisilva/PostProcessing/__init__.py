...
The typical usage pattern is to use the following classes:
    ...
    * :class:`~xpsisilva.PostProcessing.Runs` to load a set of nested sampling
      runs on one underlying posterior distribution;
    * :class:`~xpsisilva.PostProcessing.CornerPlotter` to plot 1D and 2D
      marginal posterior density distributions (from a set of
      :class:`~xpsisilva.PostProcessing.Runs` instances) in lower-triangular
      form, and to compute various posterior estimators;
    * :class:`~xpsisilva.PostProcessing.SignalPlotter` to plot thematic
    ...
    * to use the :class:`~xpsisilva.PostProcessing.SignalPlotter` class, a
      ...
      distribution to be processed, where these native classes are
        ...
        + :class:`~xpsisilva.PostProcessing.ResidualPlot`
        + :class:`~xpsisilva.PostProcessing.PulsePlot`
        + :class:`~xpsisilva.PostProcessing.SpectrumPlot`
...
base class :class:`~xpsisilva.PostProcessing._signalplot.SignalPlot`, using the
...
"""
try:
    from xpsisilva.PostProcessing._global_imports import * # access, e.g., random_seed global
except ImportError:
    from xpsisilva import _warning
