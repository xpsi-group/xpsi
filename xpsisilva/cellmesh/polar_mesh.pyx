...
from xpsisilva.cellmesh.mesh_tools cimport *
...
def construct_polar_cellMesh(size_t numThreads,
                             size_t numCell,
                             size_t sqrt_numCell,
                             double mass,
                             double r_s,
                             ...
                             double superAzimuth):
    """
    Construct a photospheric cell mesh centred on a pole, enclosing a spot.

    """
    cdef:
        signed int ii
        size_t i, j, thread
        double cellArea, mu, delta_phi
    ...
    if cedeColatitude + cedeRadius > M_PI:
        invert = 1
        colat_lims[1] = M_PI - cedeColatitude + cedeRadius
    ...
    cellArea = _2pi * integrateArea(0.0,
                                    colat_lims[1],
                                    R_eq,
                                    epsilon,
                                    zeta,
    ...
    colatInterpArray = np.linspace(colat_lims[1], colat_lims[0], 1000)
    ...
    for ii in prange(<signed int>sqrt_numCell,
        ...
        if i == 0:
            p1 = p3 = (cedeColatitude <= cedeRadius)
            if p1 == 1:
                p1 = p1 * (superRadius <= superColatitude)
                p3 = p1
            else:
                p1 = p3 = 2
    ...
    ## testing
    #total = integrateSpot(colat_lims[0],
    #                         colat_lims[1],
    #                         0.0,
    #                         _2pi,
    #                         R_eq,
    ...
    #   for j in range(sqrt_numCell):
    #       totalCellArea += cellAreas[i,j]
    #print("(Polar) spotArea / totalCellArea: %.16e" % (spotArea / totalCellArea))
    #end testing
