...
def synthesise_exposure_new(double exposure_time,
                        double[::1] phases,
                        components,
                        component_phases,
                        phase_shifts,
    ...
    for i in range(<size_t>components[0].shape[0] ):
        for j in range(<size_t>phases.shape[0] - 1):
            background[i,j] = background[i,j] * SCALE_BACKGROUND
...
def synthesise_given_total_count_number_new(double[::1] phases,
                                        double expected_star_counts,
                                        components,
                                        component_phases,
                                        phase_shifts,
    ...
    for i in range(<size_t>components[0].shape[0] ):
        for j in range(<size_t>phases.shape[0] - 1):
            ...
            background[i,j] = background[i,j] * SCALE_BACKGROUND
            EXPEC[i,j] = STAR[i,j] + background[i,j]
